...
object MainTCP extends IOApp {
  ...
  def attendNewClient(clientSocket: Socket, stopServerFlag: MVar[IO, Unit]): IO[Unit] = {
    ...
    def loop(reader: BufferedReader, writer: BufferedWriter): IO[Unit] = for {
      _     <- IO.cancelBoundary
      lineE <- reader.readLineIO.attempt
      _     <- lineE match {
  ...
  def serve(serverSocket: ServerSocket, stopServerFlag: MVar[IO, Unit]): IO[Unit] = {
    ...
    for {
      ...
      _             <- acceptNewConnections(activeClients).guarantee{ for {
                           ...
                           _       <- Traverse[List].sequence(clients.toList.map(_.cancel))
                         } yield ()
                       }
    } yield ()
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    ServerSocketIO.serverSocket(Option(5002), None, None)
      .bracketCase {
        case serverSocket => server(serverSocket)
      } {
        case (serverSocket, Completed)  => IO{ println("Server was successfully run") } *> closeServerSocket(serverSocket)
        case (serverSocket, Canceled)   => IO{ println("Server execution was cancelled") } *> closeServerSocket(serverSocket)
        case (serverSocket, Error(err)) => IO{ println(s"Server was stopped due to an error: ${err.getMessage}") } *> closeServerSocket(serverSocket)
      }
  }
